...
// First install the package with: npm install react-type-animation
...
const fadeIn = {
  ...
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.6, staggerChildren: 0.2 }, // Added staggerChildren for child animations
  },
}
...
const clientLogos = [
  ...
  },
  {
    ...
    darkLogo: "/logoipsum-3-dark.svg", // Corrected to use distinct dark logos
    width: 100,
    height: 40,
  },
  {
    ...
    darkLogo: "/logoipsum-4-dark.svg", // Corrected to use distinct dark logos
    width: 110,
    height: 40,
  },
  {
    ...
    darkLogo: "/logoipsum-5-dark.svg", // Corrected to use distinct dark logos
    width: 120,
    height: 40,
  },
  {
    ...
    darkLogo: "/logoipsum-6-dark.svg", // Corrected to use distinct dark logos
    width: 100,
    height: 40,
  },
]
...
const Hero = () => {
  ...
  // Placeholders for the typing animation
  ...
  return (
    <section className={`relative py-16 md:py-20 overflow-hidden min-h-[calc(100vh-80px)] flex flex-col justify-center
      ...
      <div className="container relative z-10 px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-12"> {/* Flex container for content and image */}
        {/* Left Section: Text and Search Bar */}
        <motion.div
          className="max-w-2xl text-center md:text-left space-y-8 md:w-1/2" // Adjusted width for flex layout
          initial="hidden"
          animate="visible"
          variants={fadeIn}
        >
          <motion.h1
            className="font-heading text-3xl md:text-4xl lg:text-6xl font-bold leading-tight md:leading-tight mb-8 text-gray-900 dark:text-white" // Adjust text colors
            variants={fadeIn}
          >
            Transform Your{" "}
            <span
          ...
          <motion.p
            className="text-lg md:text-xl text-gray-700 dark:text-gray-300 mx-auto md:mx-0 leading-relaxed" // Adjust text colors
            variants={fadeIn}
          >
          ...
          <motion.div variants={fadeIn} className="flex flex-col sm:flex-row justify-center md:justify-start gap-6 pt-8 px-0"> {/* Removed horizontal padding */}
            <div className="relative w-full max-w-xl group"> {/* Increased max-w for input relative to flex item */}
              ...
              {/* Use TypeAnimation for the placeholder */}
              <Input
        ...
        <motion.div
          className="w-full md:w-1/2 flex justify-center md:justify-end mt-12 md:mt-0" // Adjusted width for flex layout, pushed to end
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.8, delay: 0.3 }}
        >
          <Image
            src="/header-img.png" // Path to your header image
            alt="Digital presence with AI"
            width={700} // Adjust width as needed for your design
            height={500} // Adjust height as needed
            priority // Load with high priority as it's above the fold
            className="rounded-lg  object-contain w-full h-auto max-w-[600px]" // Responsive image styling
          />
        </motion.div>
      </div>
      ...
      <motion.div
        className="mt-20 pb-8 container mx-auto px-4 md:px-6" /* Increased top margin for spacing */
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.5, duration: 0.5 }}
      >
        <div className="text-center mb-6">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider"> {/* Adjusted text color */}
            Trusted by Industry Leaders
          </p>
        </div>

        <div className="relative overflow-hidden">
          <div className="flex animate-marquee gap-16 opacity-80 hover:opacity-100 transition-opacity duration-500 py-4"> {/* Added vertical padding */}
            {/* First set of logos */}
            {mounted && clientLogos.map((logo, i) => (
              ...
              >
                <Image
                  ...
                  priority={i < 3} // Prioritize first few logos
                />
              </div>
            ))}
            ...
            {mounted && clientLogos.map((logo, i) => (
              ...
              >
                <Image
                  ...
                  height={logo.height}
                  className="h-10 w-auto object-contain transition-all duration-300"
                  // No priority needed for duplicates, as they're off-screen initially
                />
              </div>
            ))}
          </div>
